...
for imember = 1 : 10
    ...
    nprm = projectdir("exp","prm","IslandRCE","control","statsrun$mbrstr.prm")
    open(tprm,"w") do fprm
        open(oprm,"r") do rprm
            s = read(rprm,String)
    ...
    @info "Creating new IslandRCE prm file for r = $(rstr), MLD = $(mldstr), member $mbrstr"
    ...
    for r in rvec, mld in mldvec
        ...
        mv(tprm,nprm,force=true)
        @info "Creating new spinup IslandRCE prm file for r = $(rstr), MLD = $(mldstr) member $mbrstr"

        oprm = projectdir("run","modifysam","prmtemplates","islandrce-stats.prm")
        nprm = projectdir("exp","prm","IslandRCE","$(fID)","statsrun$mbrstr.prm")
        open(tprm,"w") do fprm
            open(oprm,"r") do rprm
                s = read(rprm,String)
                s = replace(s,"[r]"   => rstr)
